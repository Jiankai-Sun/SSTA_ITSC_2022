...
def astar(maze, start, end, step_size=8):
...
class ImageBasedLocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict=None):
        ...
        self.ego_vehicle_location_offset=[500, 866]
        self.step_index = 0
        self.goal_distance = 20
        self.save_img_dir = 'save_img_dir_grid'
    ...
    def run_step(self, grid_img=None, debug=False):
        """
        ...
        print('1 self.target_waypoint: ', self.target_waypoint)
        ...
        end = (int(start[0]), int(start[1] + self.goal_distance))
        print('start: {}, end: {}'.format(start, end))
        # grid_img[:int(start[0]) - 10] = 1
        # grid_img[int(start[0]) + 10:] = 1
        image = copy.deepcopy(grid_img)
        image[start[0]-2:start[0]+2, start[1]-2:start[1]+2] = 1
        image[end[0] - 2:end[0] + 2, end[1] - 2:end[1] + 2] = 1
        # image = cv2.circle(image, (end[1], end[0]), radius=1, color=(0, 0, 255), thickness=-1)
        # plt.imshow(image, cmap='gray',)
        # plt.show()
        if self.save_img_dir is not None:
            cv2.imwrite(os.path.join(self.save_img_dir, 'grid_img_{0:06d}.png'.format(self.step_index)),
                        image.astype(np.float32) * 255.)
        path = astar(grid_img, start=start, end=end)
        print('path: ', path)
        target_location = self.target_waypoint.transform.location
        print('1 target location: ', target_location)
        target_location.x = (path[1][0] + self.ego_vehicle_location_offset[0]) * 0.22
        target_location.y = (path[1][1] + self.ego_vehicle_location_offset[1]) * 0.22
        print('2 target location: ', target_location)
        self.target_waypoint = self._map.get_waypoint(target_location)
        print('2 self.target_waypoint', self.target_waypoint)
